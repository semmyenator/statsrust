// Copyright (c) semmyenator statsrust Contributors
...
impl Kernel {
    /// Creates kernel from string name
    pub fn from_name(name: &str) -> Result<Self, StatsrustError> {
        match name.to_lowercase().as_str() {
            "normal" | "gauss" => Ok(Kernel::Normal),
            "logistic" => Ok(Kernel::Logistic),
            "sigmoid" => Ok(Kernel::Sigmoid),
            "rectangular" | "uniform" => Ok(Kernel::Rectangular),
            "triangular" => Ok(Kernel::Triangular),
            "parabolic" | "epanechnikov" => Ok(Kernel::Parabolic),
            "quartic" | "biweight" => Ok(Kernel::Quartic),
            "triweight" => Ok(Kernel::Triweight),
            "cosine" => Ok(Kernel::Cosine),
            _ => Err(StatsrustError::UnknownKernel(name.to_string())),
        }
    }
    
    /// Returns the kernel function
    fn kernel(&self) -> Box<dyn Fn(f64) -> f64 + Send + Sync> {
        match self {
            Kernel::Normal => Box::new(|t| (-(t * t) / 2.0).exp() / (2.0 * std::f64::consts::PI).sqrt()),
            Kernel::Logistic => Box::new(|t| 0.5 / (1.0 + t.cosh())),
            Kernel::Sigmoid => Box::new(|t| (1.0 / std::f64::consts::PI) / t.cosh()),
            Kernel::Rectangular => Box::new(|t| if t.abs() <= 1.0 { 0.5 } else { 0.0 }),
            Kernel::Triangular => Box::new(|t| if t.abs() <= 1.0 { 1.0 - t.abs() } else { 0.0 }),
            Kernel::Parabolic => Box::new(|t| if t.abs() <= 1.0 { 0.75 * (1.0 - t * t) } else { 0.0 }),
            Kernel::Quartic => Box::new(|t| if t.abs() <= 1.0 { 15.0 / 16.0 * (1.0 - t * t).powi(2) } else { 0.0 }),
            Kernel::Triweight => Box::new(|t| if t.abs() <= 1.0 { 35.0 / 32.0 * (1.0 - t * t).powi(3) } else { 0.0 }),
            Kernel::Cosine => Box::new(|t| if t.abs() <= 1.0 { (std::f64::consts::PI / 4.0) * ((std::f64::consts::PI / 2.0) * t).cos() } else { 0.0 }),
        }
    }
    
    /// Returns the inverse CDF function for random sampling
    fn inv_cdf(&self) -> Box<dyn Fn(f64) -> f64 + Send + Sync> {
        match self {
            Kernel::Normal => Box::new(|p| {
                let normal = Normal::new(0.0, 1.0).unwrap();
                normal.inverse_cdf(p)
            }),
            Kernel::Logistic => Box::new(|p| p.ln() - (1.0 - p).ln()),
            Kernel::Sigmoid => Box::new(|p| (p * std::f64::consts::PI / 2.0).tan().ln()),
            Kernel::Rectangular => Box::new(|p| 2.0 * p - 1.0),
            Kernel::Triangular => Box::new(|p| {
                if p < 0.5 {
                    (2.0 * p).sqrt() - 1.0
                } else {
                    1.0 - (2.0 - 2.0 * p).sqrt()
            ...
            Kernel::Parabolic => Box::new(|p| 2.0 * (((p * 2.0 - 1.0).acos() + std::f64::consts::PI) / 3.0).cos()),
            Kernel::Quartic => Box::new(|p| {
                let mut x = if p <= 0.5 { 
            ...
            Kernel::Triweight => Box::new(|p| {
                let sign = if p <= 0.5 { 1.0 } else { -1.0 };
                let p_adj = if p <= 0.5 { p } else { 1.0 - p };
                (2.0 * p_adj).powf(0.3400218741872791) - 1.0 * sign
            }),
            Kernel::Cosine => Box::new(|p| 2.0 * ((2.0 * p - 1.0).asin()) / std::f64::consts::PI),
        }
    }
    
    /// Returns the CDF function
    fn cdf(&self) -> Box<dyn Fn(f64) -> f64 + Send + Sync> {
        match self {
            Kernel::Normal => Box::new(|t| {
                let normal = Normal::new(0.0, 1.0).unwrap();
                normal.cdf(t)
            }),
            Kernel::Logistic => Box::new(|t| 1.0 - 1.0 / (t.exp() + 1.0)),
            Kernel::Sigmoid => Box::new(|t| (2.0 / std::f64::consts::PI) * (t.exp()).atan()),
            Kernel::Rectangular => Box::new(|t| if t < -1.0 { 0.0 } else if t > 1.0 { 1.0 } else { 0.5 * t + 0.5 }),
            Kernel::Triangular => Box::new(|t| {
                if t < -1.0 {
                    0.0
                } else if t > 1.0 {
                    1.0
    ...
    T: ToPrimitive + Copy
{
    ...
    let variance = match mu {
        Some(mu) => {
            let diff = &arr - mu;
            diff.dot(&diff) / arr.len() as f64
        }
        None => {
            arr.var(0.0)
        }
    ...
    T: ToPrimitive + Copy
{
    ...
    let values: Vec<f64> = data.iter()
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let arr = Array1::from(values);
    let variance = match xbar {
        Some(xbar) => {
            let diff = &arr - xbar;
            diff.dot(&diff) / (arr.len() - 1) as f64
        }
        None => {
            arr.var(1.0)
        }
    ...
    T: ToPrimitive + Copy + PartialOrd
{
    ...
    match method {
        ...
        _ => Err(StatsrustError::UnknownKernel(method.to_string())),
    }
}
    ...
    T: ToPrimitive + Copy
{
    ...
    match method {
        "linear" => {
            let x_mean = x_arr.mean().unwrap();
            let y_mean = y_arr.mean().unwrap();
            let x_centered = &x_arr - x_mean;
            let y_centered = &y_arr - y_mean;
            let sxy = x_centered.dot(&y_centered);
            let sxx = x_centered.dot(&x_centered);
            let syy = y_centered.dot(&y_centered);
            if sxx == 0.0 || syy == 0.0 {
        ...
        "ranked" => {
            let x_ranks = rank(&x_arr)?;
            let y_ranks = rank(&y_arr)?;
    ...
    T: ToPrimitive + Copy
{
    ...
    let x_arr = Array1::from(x);
    let y_arr = Array1::from(y);
    let x_mean = if proportional { 0.0 } else { x_arr.mean().unwrap() };
    let y_mean = if proportional { 0.0 } else { y_arr.mean().unwrap() };
    let x_centered: Array1<f64> = if proportional {
        x_arr.clone()
    } else {
        &x_arr - x_mean
    };
    let y_centered: Array1<f64> = if proportional {
        y_arr.clone()
    } else {
        &y_arr - y_mean
    };
...
fn rank<T: Float + ToPrimitive>(data: &Array1<T>) -> Result<Vec<f64>, StatsrustError> {
    let n = data.len();
    let mut indexed: Vec<(T, usize)> = data.iter().copied().zip(0..n).collect();
    indexed.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
    let mut ranks = vec![0.0; n];
    let mut i = 0;
    while i < n {
        let mut j = i;
        while j < n && data[indexed[j].1] == data[indexed[i].1] {
            j += 1;
        }
...
pub fn kde<T>(data: &[T], h: f64, kernel: &str, cumulative: bool) -> Result<Box<dyn Fn(f64) -> f64 + Send + Sync>, StatsrustError> 
where 
    T: ToPrimitive + Copy
{
    if data.is_empty() {
    ...
    let kernel_type = Kernel::from_name(kernel)?;
    let kernel_fn = kernel_type.kernel();
    let cdf_fn = kernel_type.cdf();
    let support = kernel_type.support();
    let values: Vec<f64> = data.iter()
        .map(|&x| x.to_f64().ok_or(StatsrustError::NonNumericInput))
        .collect::<Result<Vec<_>, _>>()?;
...
pub fn kde_random<T>(data: &[T], h: f64, kernel: &str, seed: Option<u64>) -> Result<Box<dyn FnMut() -> f64 + Send + Sync>, StatsrustError> 
where 
    T: ToPrimitive + Copy
{
    if data.is_empty() {
    ...
    let kernel_type = Kernel::from_name(kernel)?;
    let inv_cdf = kernel_type.inv_cdf();
    let values: Vec<f64> = data.iter()
        .map(|&x| x.to_f64().ok_or(StatsrustError::NonNumericInput))
        .collect::<Result<Vec<_>, _>>()?;
...
impl NormalDist {
    ...
    pub fn pdf(&self, x: f64) -> Result<f64, StatsrustError> {
        if self.sigma == 0.0 {
            return Err(StatsrustError::NegativeValueNotAllowed);
        }
        let normal = Normal::new(self.mu, self.sigma).unwrap();
    ...
    pub fn cdf(&self, x: f64) -> Result<f64, StatsrustError> {
        if self.sigma == 0.0 {
            return Err(StatsrustError::NegativeValueNotAllowed);
        }
        let normal = Normal::new(self.mu, self.sigma).unwrap();
    ...
    pub fn overlap(&self, other: &Self) -> f64 {
        ...
        if dv.abs() < 1e-10 {
            return 1.0 - statrs::function::erf::erf(dm / (2.0 * x.sigma * 2f64.sqrt()));
        }
        let a = x.mu * y_var - y.mu * x_var;
        let b = x.sigma * y.sigma * (dm * dm + dv * (y_var / x_var).ln()).sqrt();
        let x1 = (a + b) / dv;
        let x2 = (a - b) / dv;
        let y_cdf_x1 = y.cdf(x1).unwrap_or(0.0);
        let x_cdf_x1 = x.cdf(x1).unwrap_or(0.0);
        let y_cdf_x2 = y.cdf(x2).unwrap_or(0.0);
        let x_cdf_x2 = x.cdf(x2).unwrap_or(0.0);
        1.0 - (y_cdf_x1.abs() - x_cdf_x1.abs() + y_cdf_x2.abs() - x_cdf_x2.abs())
    }
...
mod tests {
    ...
    fn test_normal_dist() {
        ...
        assert!((dist2.mean() - 0.0).abs() < 0.2);
        assert!((dist2.stdev() - 1.0).abs() < 0.2);
    }
    
    #[test]
    fn test_kde() {
        let data = vec![1, 2, 3, 4, 5];
        let kde_func = kde(&data, 1.0, "normal", false).unwrap();
        let value = kde_func(3.0);
        assert!(value > 0.0);
        let kde_cdf = kde(&data, 1.0, "normal", true).unwrap();
    ...
    fn test_kde_random() {
        let data = vec![1, 2, 3, 4, 5];
        let mut kde_rand = kde_random(&data, 1.0, "normal", Some(42)).unwrap();
